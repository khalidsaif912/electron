...
void UtilityAILanguageModel::Prompt(
  ...
  auto SendResponse = [](base::WeakPtr<UtilityAILanguageModel> weak_ptr,
    ...
    if (result->IsString() && gin::ConvertFromV8(isolate, result, &response)) {
      ...
      return;
    } else if (IsReadableStream(isolate, result)) {
      // TODO - Create a reader, figure out lifetime for the reader, and keep
      //        pumping it for next responses until it says it's done
    } else {
      // TODO - Error handling
      responder->OnError(
